 ...
 *  11.) The "MinimumCircleSize" will set the smallest value a circle can shrink to.
 * 
 *              If you set all number parameters to 0 it will load in my "recommended" values.
 *              Also, this code runs like ass with a number of circles larger than my recommended.
 * 
 *  [OPTIONALS]
...
public class CirclePackingBreathing : MonoBehaviour {
    ...
    [SerializeField] float MinimumCircleSize;
    ...
    private List<GameObject> CirclesToShrink;
    void Start() {
        GrowSpeed = GrowSpeed <= 0f ? .01f : GrowSpeed;
        DownscalingFactor = DownscalingFactor <= 0 ? 50 : DownscalingFactor;
        MinimumCircleSize = MinimumCircleSize <= 0f ? .1f : MinimumCircleSize;
        MaximumCircleCount = MaximumCircleCount <= 0 ? 200 : MaximumCircleCount;
    ...
    private IEnumerator SpawnNewCircle() {
        ...
        while (LocationAvailable) {
            ...
            try {
                if (AllCircles.Count < MaximumCircleCount) {
    ...
    private IEnumerator ChangeCircleSize() {
        while (true) {
            ...
            for (int i = 0; i < CirclesToGrow.Count; i++) {
                ...
                for (int j = 0; j < AllCircles.Count; j++) {
                    if (CirclesToGrow[i] != AllCircles[j]) {
                        ...
                        if (Radi >= Dist) {
                            CanGrow = false;
                            CirclesToShrink.Add(CirclesToGrow[i]);
            ...
            for (int i = 0; i < CirclesToShrink.Count; i++) {
                if (CirclesToShrink[i].transform.localScale.x > MinimumCircleSize) {
                    CirclesToShrink[i].transform.localScale -= new Vector3(SizeToGrowPerFrame, SizeToGrowPerFrame, 0);
                }
                else {
                    CirclesToGrow.Add(CirclesToShrink[i]);
                    CirclesToShrink.RemoveAt(i);
                }
            }
        }
    }
