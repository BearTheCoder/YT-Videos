 ...
 *              Also, this code can handle numbers much larger than my recommended, but the bubbles "pop" before you can even see them.
 * 
 *  [OPTIONALS]
...
public class CirclePackingPop : MonoBehaviour {
    ...
    void Start() {
        ...
        DownscalingFactor = DownscalingFactor <= 0 ? 20 : DownscalingFactor;
        MaximumCircleCount = MaximumCircleCount <= 0 ? 100 : MaximumCircleCount;
        ...
        StartCoroutine("GrowAndPopCircles");
    }
    ...
    private void SpawnFirstCircle() {
        ...
        GameObject CurrentGO = Instantiate(Circle, Location, Quaternion.identity, transform);
        ...
        while (LocationAvailable) {
            ...
            try {
                if (CountOfCurrentCirlces < MaximumCircleCount) {
                    ...
                    if (CanSpawn) {
                        GameObject GO = Instantiate(Circle, Location, Quaternion.identity, transform);
                        AllCircles.Add(GO);
                        CountOfCurrentCirlces++;
                    }
                }
    ...
    private IEnumerator GrowAndPopCircles() {
        while (true) {
            yield return new WaitForSeconds(GrowSpeed);
            for (int i = 0; i < AllCircles.Count; i++) {
                bool CanGrow = true;
                for (int j = 0; j < AllCircles.Count; j++) {
                    if (AllCircles[i] != AllCircles[j]) {
                        float Dist = Vector3.Distance(AllCircles[i].transform.position, AllCircles[j].transform.position);
                        float Radi = (AllCircles[i].transform.localScale.x / 2) + (AllCircles[j].transform.localScale.x / 2);
                        if (Radi >= Dist) {
                            CanGrow = false;
                            GameObject.Destroy(AllCircles[i]);
                            AllCircles.RemoveAt(i);
                            CountOfCurrentCirlces--;
                            break;
                        }
                    }
                }
                if (CanGrow) { AllCircles[i].transform.localScale += new Vector3(SizeToGrowPerFrame, SizeToGrowPerFrame, 0); }
            }
        }
    }
}
